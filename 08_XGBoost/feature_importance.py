"""
Simple Feature Importance Visualization - XGBoost Model
...
def load_xgboost_model():
    ...
    if not model_files:
        raise FileNotFoundError("No XGBoost model files found")
...
def plot_feature_importance_horizontal(model, feature_names, top_n=15):
    ...
    # Get feature importance - try multiple methods
    importance = None
    importance_type = "unknown"
    
    # Method 1: Standard feature_importances_
    if hasattr(model, 'feature_importances_'):
        importance = model.feature_importances_
        importance_type = "sklearn_compatible"
    
    # Method 2: XGBoost booster get_score
        ...
        try:
            ...
            for feature_key, score in score_dict.items():
                # Handle feature names like 'f0', 'f1', etc.
                if feature_key.startswith('f'):
                    feature_idx = int(feature_key[1:])
                    if feature_idx < len(importance):
                        importance[feature_idx] = score
            importance_type = "xgboost_gain"
        except:
            pass
    
    # Method 3: Direct booster access
        ...
        try:
            ...
            for feature_key, score in score_dict.items():
                if feature_key.startswith('f'):
                    ...
                    if feature_idx < len(importance):
                        importance[feature_idx] = score
            importance_type = "direct_booster"
        except:
            pass
    
    if importance is None:
        raise AttributeError("Cannot extract feature importance from model")
    
    print(f"Importance extraction method: {importance_type}")
    
    # Handle zero importance
    if np.sum(importance) == 0:
        print("Warning: All importance values are zero!")
        return None
...
def main():
    ...
    try:
        ...
        model, saved_feature_names = load_xgboost_model()
